...
class AttentionConvLSTM(nn.Module):
    def __init__(self,):
        ...
        #self.position_encoding = PositionEmbeddingSine(num_pos_feats=32, temperature=10000)
    ...
    def forward(self, x):      #(b_s, 3, 300, 40)
        ...
        out = self.transformer(x4,x5)               #(b_s, 150, 640)
...
if __name__ == '__main__':
    ...
    #
        if val_acc > best_acc:
            ...
            torch.save(model.state_dict(), os.path.join(SAVE_PATH, 'covid19_pyc_transformer6_model' + str(epoch) + '.pt'))
            print('Model is saved to {}'.format(os.path.join(SAVE_PATH, 'covid19_pyc_transformer6v_model' + str(epoch) + '.pt')))
            logging.info('Model is saved to {}'.format(os.path.join(SAVE_PATH, 'covid19_pyc_transformer6_model' + str(epoch) + '.pt')))
...
#     # model = AttentionConvLSTM()
#     model.load_state_dict(torch.load(os.path.join(LOAD_PATH, 'covid19_pyc_transformer6_model10.pt')))
#     print('Model is loaded from {}'.format(os.path.join(LOAD_PATH, 'covid19_pyc_transformer6_model10.pt')))
#
...
#         if float(sum(pred_ss)) / len(pred_ss) > 0.35:
